...
class Asignacion():Sentencia() {
    ...
    override fun getArbolVisual(): TreeItem<String> {
        ...
        if (expresion != null) {
            raiz.children.add(TreeItem("${identificadoVariable!!.lexema} ${operadorAsignacion!!.lexema} ${expresion}"))
        } else {
            raiz.children.add(TreeItem("${identificadoVariable!!.lexema} ${operadorAsignacion!!.lexema} ${invocacion}"))
        }
            return raiz
        }

    override fun analizarSemantica(tablaSimbolos: TablaSimbolos, listaErrores: ArrayList<Error>, ambito: String) {
        var s = tablaSimbolos.buscarSimboloValor(identificadoVariable!!.lexema, ambito)
        if (s == null){
            listaErrores.add(Error("El campo (${identificadoVariable!!.lexema}) no existe dentro del ámbito ($ambito).", identificadoVariable!!.fila, identificadoVariable!!.columna))
        } else {
            var tipo = s.tipo
            if (expresion != null) {
